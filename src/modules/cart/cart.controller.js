...
export const addToCart = asyncHandler(async (req, res) => {
    ...
    if (existingItem) {
        ...
        if (product.stock < newQuantity) {
            throw new ApiError(
                400,
                `Cannot add ${quantity} more. Available: ${product.stock - existingItem.quantity}`
            );
        }
    ...
    logger.info(
        `Product ${productId} added to cart for user ${req.user._id}. Quantity: ${quantity}`
    );
...
export const updateCartItem = asyncHandler(async (req, res) => {
    ...
    if (quantity === 0) {
        ...
        logger.info(
            `Product ${productId} removed from cart for user ${req.user._id}`
        );
    } else {
        // Check stock
        const product = await Product.findById(productId);
        ...
        logger.info(
            `Product ${productId} quantity updated to ${quantity} for user ${req.user._id}`
        );
    }
